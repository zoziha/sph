module test_eos_m
    ...
    use config_m, only: rk, B, rho0, eos_form
...
contains
    ...
    subroutine test_p_art_water(error)
        ...
        call check(error, p, 10.300928280881005_rk, "p_art_water%p")
        if (allocated(error)) return
        call check(error, c, 1.0300949191898578_rk, "p_art_water%c")
        if (allocated(error)) return
        
        eos_form = 2
        call p_art_water(1000.0_rk, p, c)
        call check(error, p, 0.10000000000000001_rk, "p_art_water%p")
        if (allocated(error)) return
        call check(error, c, 1.0000000000000000E-002_rk, "p_art_water%c")
